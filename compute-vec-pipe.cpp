...
// goal: add two array, compare three types of program 
#include <stdio.h>
#include "utils.h"
#include <immintrin.h>
...
void compute_fn2(double* A, double* B, double* C) {
  ...
  for (int k = 0; k < M; k++) {
    Vector A_ = Vector::LoadAligned(A + k * VEC_LEN); //LoadAligned
...
int main(int argc, char** argv) {
  ...
  alignas(sizeof(double)*VEC_LEN) double A[VEC_LEN*M]; //align, trade off between memory and time
