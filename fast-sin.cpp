...
//**** why c_i doesn't need to be an input in function?
...
int main() {
  ...
  for (long i = 0; i < N; i++) {
    x[i] = (drand48()-0.5) * M_PI/2; // generate random number in [-pi/4,pi/4]
    sinx_ref[i] = 0; //set initial value as 0
    sinx_taylor[i] = 0;
    sinx_intrin[i] = 0;
    sinx_vector[i] = 0;
  }
